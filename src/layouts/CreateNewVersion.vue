  ...
  <div>
    <q-layout view="hHh Lpr lff" container style="height: 100vh">
      ...
      <q-drawer v-model="drawerLeft" show-if-above :width="300" :breakpoint="700" :bordered="false"
        ...
        <q-img class="absolute-top" src="/src/assets/dark.jpg" style="height: 150px">
      ...
      <q-page-container>
        <q-page class="bg-accent">
          <ContentPage v-if="fileContentArray.length > 0" :updateBPMN="updateBPMN" :items="fileContentArray"
            ...
            :updateSubject="updateSubject" :subject="subject" :getMultipleFiles="getMultipleFiles" :createFile="createNewVersion" />

          <!-- New button for creating a new version -->
          <q-btn @click="createNewVersion" color="primary" label="Create New Version" class="q-ma-md" />
        </q-page>
      </q-page-container>

      <!-- Popup for uploaded files -->
...
export default {
  ...
  async setup() {
    ...
    const createNewVersion = async () => {
      ...
      try {
        const result = await proxy.$editorCloudF.createNewFileVersion({
          fileId: fileId,
          name: label.value,
          title: subject.value,
        ...
        console.log('New file version created successfully:', result);
        ...
        // Optionally, you can update the UI or redirect to the new version
        // For now, we'll just reload the current page
        goBack();
      } catch (error) {
        console.error('Failed to create new file version:', error);
        $q.notify({
          type: 'negative',
          message: 'Failed to create new file version. Please try again.',
          timeout: 2000,
          position: 'top'
        });
      }
